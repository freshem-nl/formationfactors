...
2) kruskal-wallis test for 1/FF and ECs on lithoclass, stratigraphy and stratlithoclass
3) kruskal-wallis test for 1/FF and ECs on stratigraphy within each lithoclass
4) Dunn post-hoc test with Bejamini-Hochberg correction for 1/FF and ECs on lithoclass and stratigraphy
5) Dunn post-hoc test with Bejamini-Hochberg correction for 1/FF and ECs on stratigraphy within lithoclass
6) Boxplots per lithoclass per stratigraphy for 1/FF and ECs
7) Median for 1/FF and ECs per lithoclass and for stratigraphy within lithoclass based on post-hoc test grouping
...
path_results = Path("data/4-output/inverse_ff_ecs_uncertainty/dunn_test_results")
...
# calculate inverse of ff
df["inv_FF"] = 1/df[ff_col]
df["log10_inv_FF"] = np.log10(df[inv_ff_col])
...
def calc_stratlitho_medians(
    ...
    for _, row in stratlitho_combos.iterrows():
        ...
        for rule in rules_by_litho.get(litho, []):
            ...
            sub = df.loc[mask, [inv_ff_col, surfcond_col]].dropna()
            sub_log = df.loc[mask, ["log10_inv_FF", "log10_surfcond"]].dropna()
            ...
            out_rows.append({
                ...
                "members": members,
                "n": len(sub),
                "median_ff": sub[inv_ff_col].median(),
                "median_surfcond": sub[surfcond_col].median(),
                "median_log_ff": sub_log["log10_inv_FF"].median(),
                "median_log_surfcond": sub_log["log10_surfcond"].median(),
                "median_log_ff_to_linear_scale": 10 ** sub_log["log10_inv_FF"].median(),
                "median_log_surfcond_to_linear_scale": 10 ** sub_log["log10_surfcond"].median(),
                "mean_ff": sub[inv_ff_col].mean(),
                "mean_surfcond": sub[surfcond_col].mean(),
                "mean_log_ff": sub_log["log10_inv_FF"].mean(),
                "mean_log_surfcond": sub_log["log10_surfcond"].mean(),
                "mean_log_ff_to_linear_scale": 10 ** sub_log["log10_inv_FF"].mean(),
                "mean_log_surfcond_to_linear_scale": 10 ** sub_log["log10_surfcond"].mean(),
                "std_ff_linear_scale": sub[inv_ff_col].std(),
                "std_surfcond_linear_scale": sub[surfcond_col].std(),
                "std_log_ff": sub_log["log10_inv_FF"].std(),
                "std_log_surfcond": sub_log["log10_surfcond"].std(),
                "iqr_ff_linear_scale": sub[inv_ff_col].quantile(0.75) - sub[inv_ff_col].quantile(0.25),
                "iqr_surfcond_linear_scale": sub[surfcond_col].quantile(0.75) - sub[surfcond_col].quantile(0.25),
                "cov_ff_surfcond": sub[[inv_ff_col, surfcond_col]].cov().values[0,1],
                "cov_log_ff_surfcond": sub_log[["log10_inv_FF", "log10_surfcond"]].cov().values[0,1],
...
for col in [inv_ff_col, surfcond_col]:
    if col == inv_ff_col:
        colname = "ff"
    else: 
        colname ="surf_cond"
...
results.append({
    ...
    **kruskal_per_group(df, inv_ff_col, litho_col)
})
...
results.append({
    ...
    **kruskal_per_group(df, inv_ff_col, strat_col)
})
...
results.append({
    ...
    **kruskal_per_group(df, inv_ff_col, stratlitho_col)
})
...
for litho in valid_litho:
    ...
    results_litho_strat.append({
        ...
        "lithoklasse": litho,
        **kruskal_per_group(df_litho, inv_ff_col, strat_col)
    })
...
dunn_ff_litho = dunn_matrix_refined(df_litho_refined, val_col = inv_ff_col, group_col=litho_col, p_adjust="fdr_bh")
dunn_ff_strat = dunn_matrix_refined(df_strat_refined, val_col = inv_ff_col, group_col=strat_col, p_adjust="fdr_bh")
...
plot_box_per_litho(df, value_col=inv_ff_col,       value_label="inverse formation factor (1/FF)", filename_stub="box_ff_strat")
...
agg_litho = (
    ...
    .agg(
        n=("log10_inv_FF", "count"), 

        # log-scale statistics
        median_log_ff=("log10_inv_FF", "median"),
        median_log_surfcond=("log10_surfcond", "median"),
        mean_log_ff=("log10_inv_FF", "mean"),
        mean_log_surfcond=("log10_surfcond", "mean"),
        std_log_ff=("log10_inv_FF", "std"),
...
for litho in valid_litho:
    ...
    cov = df_litho[[inv_ff_col, surfcond_col]].cov().values[0,1]
    cov_litho.append({
        litho_col: litho,
        "cov_ff_surfcond": cov  
    })
...
# median lithoclass
std_litho_ff = df.groupby(litho_col)[inv_ff_col].std().reset_index(name="std_ff_linear_scale")
iqr_ff = (df.groupby(litho_col)[inv_ff_col].quantile(0.75) - df.groupby(litho_col)[inv_ff_col].quantile(0.25)).reset_index(name="interquartile_range_ff")
