"""
Script to perform statistical analysis of 1/FF and surface conductivity, and to check for differences between facies.

output of this script:
1) Normal distribution test
2) kruskal-wallis test for 1/FF and ECs on facies
3) kruskal-wallis test for 1/FF and ECs on facies within each lithoclass
4) Dunn post-hoc test with Bejamini-Hochberg correction for 1/FF and ECs on facies
5) Dunn post-hoc test with Bejamini-Hochberg correction for 1/FF and ECs on facies within lithoclass
6) Boxplots per lithoclass per facies for 1/FF and ECs
7) Median for 1/FF and ECs per lithoclass and for facies within lithoclass based on post-hoc test grouping
...
path_results = Path("data/4-output/inverse_ff_ecs_uncertainty/dunn_test_results_facies")
...
df.to_csv(rf"{path_labresults}/20260126_tbl05_Measurementdata_Full_post_processed.csv", index=False)
...
#     "grouping": "facies",
#     **kruskal_per_group(df, inv_ff_col, facies_col)
# })
...
for litho in valid_litho:
    ...
    results_litho_facies.append({
        ...
        "lithoklasse": litho,
        **kruskal_per_group(df_litho, inv_ff_col, facies_col)
    })
...
for facies in valid_facies:
    ...
    results_facies_litho.append({
        ...
        "klasse": facies,
        **kruskal_per_group(df_facies, inv_ff_col, litho_col)
    })
...
dunn_ff_facies = dunn_matrix_refined(df_facies_refined, val_col = inv_ff_col, group_col=facies_col, p_adjust="fdr_bh")
...
plot_box_per_litho(df, value_col=inv_ff_col,       value_label="inverse formation factor (1/FF)", filename_stub="box_ff")
...
# make boxplots for FF and surface conductivity
plot_box_per_facies(df, value_col=inv_ff_col,       value_label="formation factor (FF)", filename_stub="box_ff")
...
for facies in valid_facies:
    ...
    cov = df_facies[[inv_ff_col, surfcond_col]].cov().values[0,1]
    cov_facies.append({
        facies_col: facies,
        "cov_ff_surfcond": cov  
    })
